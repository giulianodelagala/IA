...
def Generacion (poblacion):
    ...
    for i in range(elitism_size, size_pobla):
        poblacion[i] = Recorrido (Cruzamiento(random.choice(pool).secuencia,
...
def Ploteo():
    ...
    PuntosRecorrido(poblacion[0].secuencia)
    plt.show()
...
while (True):
    ...
    mutacion_pc = 0.001 #porcentaje de especimenes para mutar
